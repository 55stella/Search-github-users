...
const UserInfo = () => {
  ...
  const items = [
    ...
    {
      id: 3, icon:
        <FiUserPlus className='icon' />, label:
        'following',
      value: following,
      color: 'purple'
    },
  ...
  return <section className='section'>
    <Wrapper className='section-center'>
      {items.map((item) => {
        const { id, icon, label, value, public_gists }=item
...
const Item = ({color,icon,label,value}) => {
  // const{color,icon,label, value}=props.items
